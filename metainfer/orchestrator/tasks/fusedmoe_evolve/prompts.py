...
def prepare_prompt(req, iter_dir, notebooks_dir, iteration,
    ...
    lines.append("")
    lines.append("2) evaluator.py")
    lines.append("   def evaluate(program_text: str) -> dict:")
    lines.append("     - exec() the program_text to get the kernel")
    lines.append("     - Test against PyTorch eager MoE reference")
    lines.append("     - Score: -1000 for crash/NaN, 0-50 for correctness fail,")
    lines.append("       50+50*speedup if all correctness passes")
    lines.append("     - Catch all exceptions, never crash")
    lines.append("")
