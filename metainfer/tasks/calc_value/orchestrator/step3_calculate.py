"""Step 3: per-node FLOPs / mem-traffic calc with tiered angles.

Algorithm (angle-serial, node-parallel, streaming):

1. Classify each node as ``simple`` or ``complex`` based on its
   operator. Simple nodes (layernorm, residual add, embedding lookup,
   elementwise ops) only need ONE angle — there's no shape or
   formula ambiguity to triangulate. Complex nodes (attention, MLP,
   quantized matmul, RoPE, paged KV-cache) still get TWO angles so
   we catch cross-angle disagreement.

2. For each round (up to MAX_ROUNDS_PER_NODE):
   - For each angle in [a, b] STRICTLY SERIALLY:
     - Launch ALL pending nodes THAT INCLUDE THIS ANGLE in parallel.
       Simple nodes only spawn for angle a.
     - As each agent finishes, run its calc.py once at the canonical
       shape, write the cell (calc.py / response.txt / result.json)
       + update _state.json + emit timeline.
   - After all angles done for this round: find nodes whose available
     angles disagree > REL_TOL (only applies to complex nodes).
     Those become the pending set for next round.

3. Converged nodes (simple nodes auto-converge after angle a, complex
   nodes converge when both angles agree): pick angle a's script as
   canonical, write to final/<compound>.py + .meta.json.

4. Nodes still disputed after MAX_ROUNDS_PER_NODE: mark approximate,
...
values via /calc/compute.

Memory slicing: each writer only sees the operator_calls /
uncertainties relevant to ITS node (see :mod:`memory_slice`),
shrinking the per-node prompt from ~80KB to ~3KB. For 43 nodes that's
~3.4MB → ~130KB of memory-in-prompt bytes per round.
...
Output layout::
    ...
    │       └── b/round_NN/...         # only for complex nodes
    ├── final/
...
# Operators that warrant two-angle triangulation. These have either
# (a) non-trivial shape arithmetic (attention, RoPE, paged KV), or
# (b) formula ambiguity between FLOPs and memory-access accounting
# (matmul-heavy MLPs, quantized linears). Anything NOT matching these
# substrings runs with one angle only — saves a full agent spawn and
# the dispute-resolution round for the ~half of nodes that are simple
# elementwise / reduction ops.
_COMPLEX_OP_PATTERNS = (
    "attention", "attn", "flash",
    "mlp", "matmul", "linear", "gate_proj", "up_proj", "down_proj",
    "quant", "dequant", "int8", "int4", "fp8", "awq", "gptq",
    "rope", "rotary",
    "conv",  # 1d / 2d / depthwise
    "paged", "kv_cache", "kv-cache",
    "moe", "expert", "router",  # mixture-of-experts routing
)


def _classify_complexity(node: Dict[str, Any]) -> bool:
    """Return True if the node needs two-angle triangulation.

    Searches the node JSON for any operator category pattern. Hit on
    any of them → complex (2 angles). Miss on all → simple (1 angle).

    The classifier is intentionally over-broad: false positives
    (classifying a simple node as complex) cost an extra angle but
    never produce wrong results. False negatives (classifying a
    complex node as simple) could let a bad formula through unnoticed.
    So when in doubt, classify UP.
    """
    blob = json.dumps(node, ensure_ascii=False).lower()
    return any(pat in blob for pat in _COMPLEX_OP_PATTERNS)
...
class CellStateStore:
    ...
    def init_node(self, compound: str, *, node_id: str, section_id: str,
                  section_kind: Optional[str], section_repeat_count: int,
                  complex_node: bool = True) -> None:
        with self._lock:
            if compound not in self._doc["nodes"]:
                # Which angles this node will spawn. Simple nodes (1
                # angle) auto-converge when angle "a" lands; complex
                # nodes need both angles before _recompute_locked can
                # judge convergence.
                angles_for_node = list(ANGLES) if complex_node else [ANGLES[0]]
                self._doc["nodes"][compound] = {
                    "node_id": node_id,
                    "section_id": section_id,
                    "section_kind": section_kind,
                    "section_repeat_count": section_repeat_count,
                    "complex": bool(complex_node),
                    "angles": angles_for_node,
                    "cells": {
                        a: {
    ...
    def _recompute_locked(self, compound: str) -> None:
        ...
        if not node:
            return
        cells = node["cells"]
        angles_for_node = node.get("angles") or list(ANGLES)
        # Convergence is based on prefill.tflops — same convention as the
        # 5% relative tolerance was originally calibrated for.
        vals = [cells[a] for a in angles_for_node
                if isinstance(cells.get(a), dict)
                and cells[a].get("tflops") is not None]
        # Simple nodes (1 angle) auto-converge as soon as the single
        # angle produces a value — no triangulation needed.
        if len(angles_for_node) == 1:
            if vals:
                node["converged"] = True
                node["spread_pct"] = 0.0
            else:
            ...
            return
        # Complex (>=2 angles): compare across angles.
        if len(vals) < len(angles_for_node):
            node["converged"] = None
            node["spread_pct"] = None
            return
...
def _run_angle_stage(
    *,
    ...
    Skips nodes whose angle-tier doesn't include this angle (simple
    nodes only run for angle ``a``). Each writer receives a sliced
    memory (~2-3KB) scoped to its own operator rather than the full
    memory.json.
    ...
    for node_meta in pending:
        ...
        # Skip nodes whose tier doesn't include this angle. Simple
        # nodes have angles == ["a"] — when angle == "b" we no-op.
        if angle not in node_meta.get("angles", ANGLES):
            continue
...
def _find_disputed(
    *,
    ...
    """For each pending node, read back this round's results from all
    angles the node spawns and check agreement. Returns:
    ...
    Simple nodes (1 angle) never reach here as "disputed" — they have
    nothing to disagree with. They land in bad_pending only if their
    single cell failed to produce a result.json.
    """
    ...
    for node_meta in pending:
        compound = node_meta["compound"]
        angles_for_node = node_meta.get("angles") or list(ANGLES)
        results: List[Dict[str, Any]] = []
        script_for_node: Optional[str] = None
        ok = True
        for angle in angles_for_node:
            cell_dir = cells_root / compound / angle / f"round_{round_idx:02d}"
        ...
        if not ok or len(results) < len(angles_for_node):
            bad.append(node_meta)
            mismatches_by_node[compound] = []
            continue
        # Single-angle nodes can't dispute — only multi-angle ones can.
        if len(angles_for_node) < 2:
            continue
...
def _finalize_node(
    *,
    ...
    Strategy: read all angles' scripts from the last completed round
    (one angle for simple nodes, two for complex ones). If cell_state
    says converged → take writer_0 (arbitrary, all agree).
    Else → take the writer whose single-shape result is closest to the
    median (only applies to complex nodes; simple nodes have nothing
    to median over).
    """
    angles_for_node = node_meta.get("angles") or list(ANGLES)
    # Find the latest round where all angles-for-this-node produced a result.
    chosen_round = -1
    ...
    for r in range(last_round, -1, -1):
        ...
        if ok and len(trial) == len(angles_for_node):
            chosen_round = r
    ...
    if len(angles_for_node) < 2:
        # Simple node: one angle only — no comparison possible.
        chosen_angle = angles_for_node[0]
        approximate = False
        source_agent = "single_angle"
        comparison = {"ok": True, "mismatches": []}
    else:
    ...
    meta = {
        ...
        "compound_id": compound,
        "complex": node_meta.get("complex", True),
        "angles": list(angles_for_node),
...
def run_step3_calculate(
    *,
    ...
    # Flatten (section, node) targets + classify each node's complexity.
    ...
    for sec in graph.get("sections") or []:
        ...
        for node in (sec.get("graph") or {}).get("nodes") or []:
            ...
            complex_node = _classify_complexity(node)
            if complex_node:
                n_complex += 1
            else:
                n_simple += 1
            angles = list(ANGLES) if complex_node else [ANGLES[0]]
            targets.append({
                "node": node,
                "node_id": nid_raw,
                "section_id": sid,
    ...
    store.append_timeline(
        "calc_value.s3.start",
        {"nodes": total,
         "simple_nodes": n_simple,
         "complex_nodes": n_complex,
