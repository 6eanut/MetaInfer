...
were originally task-agnostic in the shell, but the iteration record
schema (``perf``, ``goal``, ``start_phase``, ``retrospective_path``,
``failure_reason``, ...) is fundamentally calc-shaped — so they've been
moved here as part of the shell shrink.

Each task package that wants a similar panel ships its own copy; if
patterns crystallize across ≥3 tasks, extraction back into a shared
helper is fine. For now: copy-first, don't pre-abstract.
"""
...
def read_state_graph(state_dir: Path) -> Dict[str, Any]:
    """Dispatch to calc_value's own ``phases.graph_payload``.

    The shell no longer hosts this endpoint — the data shape (nodes /
    edges / outcome legend) is task-specific."""
    ...
    if not hasattr(_phases, "graph_payload"):
        return {"error": "calc phases module does not export graph_payload()"}
