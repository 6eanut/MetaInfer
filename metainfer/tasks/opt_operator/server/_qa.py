"""QA config for the opt_operator task type.

Resolves an offline-QA target from either an explicit ``events_file`` or a
``(iteration, agent)`` pair. Mirrors the shared resolver contract used by the
other task types.
"""
...
def _resolve_events_file(state_dir: Path, iteration: int, agent: str) -> Path:
    ...
    for d in [
        ...
        state_dir / "logs" / NNN / ".metainfer-logs" / agent,
