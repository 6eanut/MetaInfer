...
def test_reset_preserves_requirements_json(tmp_path):
    ...
    (tmp_path / "orchestrator.log").write_text("blah", encoding="utf-8")
    (tmp_path / "orchestrator.pid").write_text("12345", encoding="utf-8")
    (tmp_path / "iterations").mkdir()
    (tmp_path / "step3").mkdir()
    (tmp_path / "timeline.jsonl").write_text("", encoding="utf-8")

    summary = sr.reset_state_dir(tmp_path, "tid-1", "calc-theoretical-value")

    # requirements.json survived.
    assert sr.read_requirements(tmp_path) == req
    # Everything else is gone.
    assert not (tmp_path / "run.json").exists() or sr.read_run(tmp_path)["finished"] is False
    assert not (tmp_path / "orchestrator.log").exists()
    assert not (tmp_path / "orchestrator.pid").exists()
    assert not (tmp_path / "iterations").exists()
    assert not (tmp_path / "step3").exists()
    # Fresh run.json was written.
    run = sr.read_run(tmp_path)
...
def test_reset_stamps_task_reset_timeline_event(tmp_path):
    ...
    sr.reset_state_dir(tmp_path, "tid-2", "calc-theoretical-value")
    events = sr.read_timeline(tmp_path)
...
def test_reset_creates_state_dir_if_missing(tmp_path):
    ...
    sr.reset_state_dir(sd, "tid-3", "calc-theoretical-value")
...
def test_reset_default_keeps_only_requirements_when_others_absent(tmp_path):
    ...
    summary = sr.reset_state_dir(tmp_path, "tid-4", "calc-theoretical-value")
    assert summary["removed"] == []
    # The file we kept is unchanged.
    assert sr.read_requirements(tmp_path) == {"task_type": "calc-theoretical-value"}
